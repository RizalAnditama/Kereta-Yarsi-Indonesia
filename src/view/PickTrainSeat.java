...
public class PickTrainSeat extends javax.swing.JFrame {
    ...
    private JToggleButton[] buttons = new JToggleButton[32];
    private int[] chosenSeatIndex;
    ...
    public PickTrainSeat() {
        ...
        panelEconomic.setVisible(true);
        panelEksekutif.setVisible(false);
    }

    public PickTrainSeat(UpdateSeat updateSeat, int panelIndex, int[] chosenSeatIndex, int currentSeatIndex) {
        this();
        this.updateSeat = updateSeat;
    ...
    private void setInitButton() {
        buttons = new JToggleButton[]{
            ...
            jToggleButton9, jToggleButton10, jToggleButton11, jToggleButton12,
            jToggleButton13, jToggleButton14, jToggleButton15, jToggleButton16
        };

        for (int i = 0; i < buttons.length; i++) {
            // Check if the current index is in the chosenSeatIndex array
            for (int j = 0; j < chosenSeatIndex.length; j++) {
                if (chosenSeatIndex[j] == i) {
                    buttons[i].setEnabled(false); // Disable already taken seats
                    buttons[i].setText("<html><font color = red>" + buttons[i].getText() + "</font></html>");
                }
            }
...
//            buttons[currentSeatIndex].setSelected(true);
//            buttons[currentSeatIndex].setBackground(Color.yellow); // Or any other color to indicate it's selected
//        }
    }

    private void setKursi(int i) {
        if (buttons[i].getBackground() == Color.red) {
            con.infoMessage("Kursi sudah dipakai!");
            buttons[i].setSelected(false);
        } else {
            if (currentSeatIndex != -1) {
                buttons[currentSeatIndex].setBackground(null); // Reset previous seat color
            }

            updateSeat.updateSeat(panelIndex, buttons[i].getText());
            updateSeat.updateSeatIndex(panelIndex, i);
            buttons[i].setBackground(Color.red); // Mark new seat as taken
            currentSeatIndex = i;
            this.dispose();
        }
    }
