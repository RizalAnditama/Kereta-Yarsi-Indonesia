...
public class PaymentMethod extends javax.swing.JFrame {
    ...
    public PaymentMethod() {
        ...
        Train train = session.getTrain();
        double subtotalHarga = session.getPenumpangTotal() * train.getPrice();
        double potonganAsuransi = session.getPotonganAsuransi() * subtotalHarga / 100;
        double potonganBiayaPenanganan = session.getPotonganBiayaPenanganan() * subtotalHarga / 100;
        double totalHarga = subtotalHarga + potonganAsuransi + potonganBiayaPenanganan;
        String kodePembayaran;

        session.setPotonganAsuransi(potonganAsuransi);
        session.setPotonganBiayaPenanganan(potonganBiayaPenanganan);
        session.setSubtotalHarga(subtotalHarga);
        session.setTotalHarga(totalHarga);

        labelHarga.setText("Total : Rp. " + nf.format(totalHarga));
    }
    ...
    private void buttonBuyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonBuyActionPerformed
        // TODO add your handling code here:
        if (getSelectedMetodePembayaran() == null) {
            con.infoMessage("Harap pilih metode pembayaran anda!");
        } else {
            session.setMetodePembayaran(getSelectedMetodePembayaran());
